...
RSpec.describe RestockReturnedProduct do
  ...
  let!(:returned_inventory1) { create(:inventory, product: product, status: :returned, order: order) }
  let!(:returned_inventory2) { create(:inventory, product: product, status: :returned, order: order) }
  
  describe '.run' do
    it 'changes returned inventory status to on_shelf' do
      RestockReturnedProduct.run(employee, product)
      
      expect(returned_inventory1.reload.status).to eq('on_shelf')
      expect(returned_inventory2.reload.status).to eq('on_shelf')
    end
    ...
    context 'when there are no returned inventories' do
      before do
        returned_inventory1.update!(status: :on_shelf)
        returned_inventory2.update!(status: :on_shelf)
      end
      ...
      it 'does not create any inventory status changes' do
        expect {
          RestockReturnedProduct.run(employee, product)
        }.not_to change(InventoryStatusChange, :count)
      end
    end
  end
end 